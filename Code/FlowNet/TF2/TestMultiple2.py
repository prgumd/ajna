...
def CreateFolders(Args):
    if not os.path.exists(Args.WritePath + 'Unc'):
        os.makedirs(Args.WritePath + 'Unc')
    if not os.path.exists(Args.WritePath + 'NormUnc'):
        os.makedirs(Args.WritePath + 'NormUnc')
    if not os.path.exists(Args.WritePath + 'Flow'):
        os.makedirs(Args.WritePath + 'Flow')
    if not os.path.exists(Args.WritePath + 'Mag'):
...
def TestOperation(InputPH, I1PH, I2PH, Label1PH, Label2PH, Args):
    ...
    with tf.compat.v1.Session() as sess: 
        ...
        for count in range(Start, NumImages-Args.Skip):
            Img1 = Args.ImgPath + 'Image%04d'%(Args.Skip*count)+ Args.ImgFormat
            Img2 = Args.ImgPath + 'Image%04d'%(Args.Skip*(count+1))+ Args.ImgFormat
            print('Computing Unc. between frames: ', Args.Skip*count, Args.Skip*(count+1))
            print('frame%06d'%(Args.Skip*count))
            print(round(count*Args.Skip*100/NumImages),'%: ','[', "█"*round(count*Args.Skip*100/NumImages),']')
            ...
            if(Args.UncType == 'Aleatoric' or Args.UncType == 'Inlier' or Args.UncType == 'LinearSoftplus'):
                ...
                cv2.imwrite(Args.WritePath + 'Unc/frame%06d'%(count) + Args.ImgFormat, np.hstack((BUncDisp[:,:,0], BUncDisp[:,:,1])))
                cv2.imwrite(Args.WritePath + 'NormUnc/frame%06d'%(count) + Args.ImgFormat, np.hstack((UncNormFlow[:,:,0], UncNormFlow[:,:,1])))
                cv2.imwrite(Args.WritePath + 'Flow/frame%06d'%(count) + Args.ImgFormat, BDisp)
                cv2.imwrite(Args.WritePath + 'Mag/frame%06d'%(count) + Args.ImgFormat, FlowMag)
                if(Args.Vid):
...
def main():
    ...
    Parser.add_argument('--CheckPointPath', default='/home/tanujthakkar/PRG/prgeyeomni/Trained/SL1ResNetSoftplusLR1e-4/399model.ckpt', \
    ...
    Parser.add_argument('--WritePath', default='/home/tanujthakkar/tmp/', help='Image Path, Default: /home/nitin/BlenderScenes/Frames6Out/')
