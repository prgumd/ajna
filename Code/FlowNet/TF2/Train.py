...
def TensorBoard(loss, I1PH, I2PH, prVal, Label1PH, Label2PH, Args):
    ...
    tf.compat.v1.summary.image('I1Patch', I1PH[:,:,:,0:3], max_outputs=3)
    ...
    tf.compat.v1.summary.image('Label1', Label1FlowVis[:,:,:,0:3], max_outputs=3)
    tf.compat.v1.summary.image('Label2', Label2FlowVis[:,:,:,0:3], max_outputs=3)
    tf.compat.v1.summary.image('prVal', prFlowVis[:,:,:,0:3], max_outputs=3)
    ...
    if(Args.UncType == 'Aleatoric'):
        ...
        tf.compat.v1.summary.image('AleatoricUncX', tf.exp(prLogvarX), max_outputs=3)
        tf.compat.v1.summary.image('AleatoricUncY', tf.exp(prLogvarY), max_outputs=3)
        ...
        tf.compat.v1.summary.image('Inlier', prVal[:,:,:,2:3], max_outputs=3)
        tf.compat.v1.summary.histogram('Inlier', prVal[:,:,:,2:3])
    elif(Args.UncType == 'LinearSoftplus'):
        Eps = 1e-3
        MaxVal = 1e9
        # Sigmoid
        # tf.compat.v1.summary.image('ScaleX', tf.clip_by_value(1/tf.math.sigmoid(prVal[:,:,:,2:3] + Eps), -MaxVal, MaxVal))
        # tf.compat.v1.summary.image('ScaleY', tf.clip_by_value(1/tf.math.sigmoid(prVal[:,:,:,3:4] + Eps), -MaxVal, MaxVal))
        # tf.compat.v1.summary.histogram('Scale', tf.clip_by_value(1/tf.math.sigmoid(prVal + Eps), -MaxVal, MaxVal))
        # Softplus
        tf.compat.v1.summary.image('ScaleX', tf.clip_by_value(1/tf.math.softplus(prVal[:,:,:,2:3] + Eps), -MaxVal, MaxVal))
        tf.compat.v1.summary.image('ScaleY', tf.clip_by_value(1/tf.math.softplus(prVal[:,:,:,3:4] + Eps), -MaxVal, MaxVal))
